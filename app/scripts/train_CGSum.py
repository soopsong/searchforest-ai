...
from app.data_util.dataloader import ScisummGraphLoader, PAD_TOKEN
from app.model.loss import SummLoss
from app.model.metric import FastRougeMetric
from app.model.model import CGSum
from app.model.callback import TrainCallback, LRDecayCallback
from app.data_util.logging import logger

def save_pickle(obj, path):
    """파이썬 객체 obj를 지정된 경로 path에 피클로 저장."""
...
def set_up_data(config, args):
    # 전처리 결과를 저장할 폴더
    os.makedirs("../../../Downloads/preprocessed", exist_ok=True)

    # 각 피클 파일 경로
    train_pkl = f"preprocessed/{args.setting}_datainfo.pkl"
    vocab_pkl = f"preprocessed/{args.setting}_vocabs.pkl"
    
    if os.path.exists(train_pkl) and os.path.exists(vocab_pkl):
        # 이미 전처리 결과가 저장되어 있으면, 피클 로드
        datainfo = load_pickle(train_pkl)
        vocabs = load_pickle(vocab_pkl)
    else:
        # 피클이 없으면 새로 전처리 → 저장
        paths = {
        ...
        logger.info(f"--- Start preprocessing for setting={args.setting} ---")
        
        datainfo, vocabs = ScisummGraphLoader(setting=args.setting).process(paths, config, args.load_vocab)
        
        # 전처리 끝났으니 피클로 저장
        save_pickle(datainfo, train_pkl)
        save_pickle(vocabs, vocab_pkl)
        logger.info(f"--- Finished preprocessing and pickled data ---")
    
    return datainfo, vocabs


def run_train(config, args, device):
    datainfo, vocabs = set_up_data(config, args)
...
if __name__ == '__main__':
    ...
    if args.visible_gpu != -1:
        ...
        device = torch.device(f"cuda:{args.visible_gpu}")
    else:
        config.use_gpu = False


    print(">>> Device for training is set to:", device)

    mode = args.mode
    logger.info("------start mode train------")
    run_train(config, args, device)