...
def main(abstract_dir: str, output_path: str, k: int, max_df: float, min_df: int, max_features: int):
    # 1) 모든 abstract 텍스트 읽어오기
    all_texts = []
    ...
    print(f"[1] Loaded {len(all_texts)} abstracts from {abstract_dir!r}")
    ...
    # 3) fit & transform
    print("[2] Fitting TF-IDF vectorizer on abstracts...")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Extract global top-N TF-IDF keywords from a directory of abstract .txt files"
    )
    p.add_argument(
        "--abstract_dir",
        required=True,
        help="Path to directory containing .txt abstract files"
    )
    p.add_argument(
        "--output",
